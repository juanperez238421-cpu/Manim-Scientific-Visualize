#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Geometry 8 - Circle fundamentals, notebook-copy edition.

Focus: center, radius, diameter, chord and arc.
Target: Manim Community Edition 0.20.1 + JP Classroom style.
Pedagogical intent: large projector-safe text and deliberate copy pauses.
"""
from __future__ import annotations

import numpy as np
from manim import *
from jp_classroom_style import *

R = 1.75
COPY_PAUSE = 9.0
THINK_PAUSE = 5.0


class Geometry8CircleFundamentalsNotebook(JPMathClassroomScene):
    def validate_lesson_data(self) -> None:
        assert_close(2 * R, 3.5, label="diameter from radius")
        assert 0 < 100 < 180
        assert 180 < 260 < 360

    def construct(self) -> None:
        self.opening()
        self.circle_and_center()
        self.radius()
        self.diameter()
        self.chord()
        self.arc()
        self.compare_all()
        self.notebook_check()

    def base_circle(self, center=np.array([-3.7, -0.45, 0.0]), radius=R):
        circle = Circle(
            radius=radius,
            stroke_color=BLACK_LINE,
            stroke_width=4.0,
            fill_color=WHITE,
            fill_opacity=1.0,
        ).move_to(center)
        return circle

    def point_with_label(self, point, label, direction=UR):
        dot = Dot(point, radius=0.075, color=BLACK_LINE)
        text = self.math(label, 31).next_to(dot, direction, buff=0.10)
        return VGroup(dot, text)

    def copy_card(self, term: str, lines: list[str], formula: str | None = None):
        title = self.text("COPY TO NOTEBOOK", 25, BOLD)
        term_m = self.text(term, 34, BOLD)
        body = VGroup(*[self.text(line, 25) for line in lines])
        body.arrange(DOWN, aligned_edge=LEFT, buff=0.16)
        content = VGroup(title, term_m, body).arrange(DOWN, aligned_edge=LEFT, buff=0.24)
        if formula is not None:
            eq = self.math(formula, 40)
            content.add(eq)
            content.arrange(DOWN, aligned_edge=LEFT, buff=0.24)
        self.fit(content, 5.85, 3.75)
        box = RoundedRectangle(
            width=6.25,
            height=4.55,
            corner_radius=0.14,
            stroke_color=BLACK_LINE,
            stroke_width=2.2,
            fill_color=WHITE,
            fill_opacity=1.0,
        )
        content.move_to(box).align_to(box, LEFT).shift(RIGHT * 0.28)
        return VGroup(box, content)

    def layout_pair(self, diagram: Mobject, copy: Mobject):
        diagram.move_to(LEFT * 3.65 + DOWN * 0.35)
        copy.move_to(RIGHT * 3.75 + DOWN * 0.35)
        group = VGroup(diagram, copy)
        self.assert_content_safe(group, "circle notebook two-column layout")
        return group

    def opening(self) -> None:
        self.standard_opening(
            "GEOMETRY 8",
            "CIRCLE FUNDAMENTALS",
            "Center, radius, diameter, chords and arcs",
            "Watch the figure, copy the definition, then connect the ideas.",
        )

    def circle_and_center(self) -> None:
        self.set_header(
            1,
            "THE CIRCLE AND ITS CENTER",
            "Start from the fixed point that organizes every other element.",
        )
        c = self.base_circle()
        O = c.get_center()
        center = self.point_with_label(O, "O", UR)
        radii = VGroup(
            Line(O, c.point_at_angle(20 * DEGREES), color=MID_GRAY, stroke_width=2.0),
            Line(O, c.point_at_angle(145 * DEGREES), color=MID_GRAY, stroke_width=2.0),
            Line(O, c.point_at_angle(255 * DEGREES), color=MID_GRAY, stroke_width=2.0),
        )
        diagram = VGroup(c, radii, center)
        card = self.copy_card(
            "CENTER (O)",
            [
                "The center is a fixed point inside the circle.",
                "Every point on the circumference is the same distance from O.",
            ],
        )
        self.layout_pair(diagram, card)
        self.play(Create(c), run_time=RUN_SLOW)
        self.play(FadeIn(center), run_time=RUN_NORMAL)
        self.play(LaggedStart(*[Create(r) for r in radii], lag_ratio=0.16), run_time=RUN_SLOW)
        self.play(FadeIn(card), run_time=RUN_NORMAL)
        self.wait(COPY_PAUSE)
        self.clear_stage()

    def radius(self) -> None:
        self.set_header(
            2,
            "RADIUS",
            "A radius starts at the center and ends on the circumference.",
        )
        c = self.base_circle()
        O = c.get_center()
        A = c.point_at_angle(28 * DEGREES)
        seg = Line(O, A, color=BLACK_LINE, stroke_width=6.0)
        labels = VGroup(
            self.point_with_label(O, "O", DL),
            self.point_with_label(A, "A", UR),
            self.math("r", 38).next_to(seg, UP, buff=0.12),
        )
        diagram = VGroup(c, seg, labels)
        card = self.copy_card(
            "RADIUS (r)",
            [
                "A radius is a segment from the center to the circumference.",
                "All radii of the same circle have equal length.",
            ],
            formula=r"OA=r",
        )
        self.layout_pair(diagram, card)
        self.play(Create(c), run_time=RUN_NORMAL)
        self.play(Create(seg), FadeIn(labels), run_time=RUN_SLOW)
        self.play(FadeIn(card), run_time=RUN_NORMAL)
        self.wait(COPY_PAUSE)
        self.clear_stage()

    def diameter(self) -> None:
        self.set_header(
            3,
            "DIAMETER",
            "A diameter is the longest chord because it passes through the center.",
        )
        c = self.base_circle()
        O = c.get_center()
        A = c.point_at_angle(180 * DEGREES)
        B = c.point_at_angle(0 * DEGREES)
        diam = Line(A, B, color=BLACK_LINE, stroke_width=6.0)
        half_l = Line(A, O, color=MID_GRAY, stroke_width=3.0)
        half_r = Line(O, B, color=MID_GRAY, stroke_width=3.0)
        labels = VGroup(
            self.point_with_label(A, "A", UL),
            self.point_with_label(O, "O", DOWN),
            self.point_with_label(B, "B", UR),
            self.math("d", 40).next_to(diam, UP, buff=0.16),
        )
        diagram = VGroup(c, half_l, half_r, diam, labels)
        card = self.copy_card(
            "DIAMETER (d)",
            [
                "A diameter joins two points on the circumference.",
                "It passes through the center of the circle.",
                "One diameter contains two radii.",
            ],
            formula=r"d=2r\qquad r=\frac{d}{2}",
        )
        self.layout_pair(diagram, card)
        self.play(Create(c), run_time=RUN_NORMAL)
        self.play(Create(diam), FadeIn(labels), run_time=RUN_SLOW)
        self.play(FadeIn(card), run_time=RUN_NORMAL)
        self.wait(COPY_PAUSE + 2.0)
        self.clear_stage()

    def chord(self) -> None:
        self.set_header(
            4,
            "CHORD",
            "A chord connects two points on the circumference; it does not have to pass through the center.",
        )
        c = self.base_circle()
        O = c.get_center()
        A = c.point_at_angle(140 * DEGREES)
        B = c.point_at_angle(25 * DEGREES)
        chord = Line(A, B, color=BLACK_LINE, stroke_width=6.0)
        diameter = Line(c.point_at_angle(180 * DEGREES), c.point_at_angle(0), color=MID_GRAY, stroke_width=2.4)
        labels = VGroup(
            self.point_with_label(A, "A", UL),
            self.point_with_label(B, "B", UR),
            self.point_with_label(O, "O", DOWN),
        )
        diagram = VGroup(c, diameter, chord, labels)
        card = self.copy_card(
            "CHORD",
            [
                "A chord is a segment joining two points on the circumference.",
                "A chord may or may not pass through the center.",
                "Every diameter is a chord, but not every chord is a diameter.",
            ],
        )
        self.layout_pair(diagram, card)
        self.play(Create(c), run_time=RUN_NORMAL)
        self.play(Create(diameter), run_time=RUN_QUICK)
        self.play(Create(chord), FadeIn(labels), run_time=RUN_SLOW)
        self.play(FadeIn(card), run_time=RUN_NORMAL)
        self.wait(COPY_PAUSE + 2.0)
        self.clear_stage()

    def arc(self) -> None:
        self.set_header(
            5,
            "ARC",
            "An arc is a curved part of the circumference between two points.",
        )
        center = np.array([-3.7, -0.45, 0.0])
        c = Circle(radius=R, stroke_color=MID_GRAY, stroke_width=2.4).move_to(center)
        start = 25 * DEGREES
        minor_angle = 100 * DEGREES
        A = c.point_at_angle(start)
        B = c.point_at_angle(start + minor_angle)
        minor = Arc(radius=R, start_angle=start, angle=minor_angle, arc_center=center, color=BLACK_LINE, stroke_width=8.0)
        major = Arc(radius=R, start_angle=start + minor_angle, angle=TAU - minor_angle, arc_center=center, color=BLACK_LINE, stroke_width=3.0)
        labels = VGroup(
            self.point_with_label(A, "A", UR),
            self.point_with_label(B, "B", UL),
            self.text("minor arc", 24, BOLD).next_to(minor.point_from_proportion(0.48), UP, buff=0.16),
            self.text("major arc", 22).next_to(major.point_from_proportion(0.54), DOWN, buff=0.16),
        )
        diagram = VGroup(c, major, minor, labels)
        card = self.copy_card(
            "ARC",
            [
                "An arc is part of the circumference between two points.",
                "A minor arc measures less than 180 degrees.",
                "A major arc measures more than 180 degrees.",
            ],
        )
        self.layout_pair(diagram, card)
        self.play(Create(c), run_time=RUN_NORMAL)
        self.play(Create(minor), FadeIn(labels[0:3]), run_time=RUN_SLOW)
        self.wait(THINK_PAUSE)
        self.play(Create(major), FadeIn(labels[3]), run_time=RUN_SLOW)
        self.play(FadeIn(card), run_time=RUN_NORMAL)
        self.wait(COPY_PAUSE + 2.0)
        self.clear_stage()

    def compare_all(self) -> None:
        self.set_header(
            6,
            "PUT THE FIVE ELEMENTS TOGETHER",
            "Identify by shape and by the points each element connects.",
        )
        center = np.array([-3.8, -0.45, 0.0])
        c = Circle(radius=1.85, stroke_color=BLACK_LINE, stroke_width=4.0).move_to(center)
        O = center
        Rpt = c.point_at_angle(42 * DEGREES)
        D1, D2 = c.point_at_angle(180 * DEGREES), c.point_at_angle(0)
        C1, C2 = c.point_at_angle(125 * DEGREES), c.point_at_angle(20 * DEGREES)
        rad = Line(O, Rpt, color=BLACK_LINE, stroke_width=4.8)
        diam = Line(D1, D2, color=BLACK_LINE, stroke_width=3.0)
        chord = Line(C1, C2, color=BLACK_LINE, stroke_width=6.0)
        arc = Arc(radius=1.85, start_angle=215 * DEGREES, angle=85 * DEGREES, arc_center=center, color=BLACK_LINE, stroke_width=8.0)
        labels = VGroup(
            self.point_with_label(O, "O", DOWN),
            self.text("radius", 23, BOLD).next_to(rad, UL, buff=0.10),
            self.text("diameter", 23).next_to(diam, DOWN, buff=0.10),
            self.text("chord", 23, BOLD).next_to(chord, UP, buff=0.10),
            self.text("arc", 23, BOLD).next_to(arc.point_from_proportion(0.50), DOWN, buff=0.12),
        )
        diagram = VGroup(c, diam, chord, rad, arc, labels)
        route = self.process_map(
            [
                ("1", "CENTER: fixed point O"),
                ("2", "RADIUS: center to circle"),
                ("3", "DIAMETER: through center"),
                ("4", "CHORD: two endpoints"),
                ("5", "ARC: curved boundary"),
            ],
            columns=1,
            card_width=6.15,
            card_height=0.73,
        )
        self.fit(route, 6.2, 4.6)
        self.layout_pair(diagram, route)
        self.play(Create(c), run_time=RUN_NORMAL)
        self.play(LaggedStart(Create(rad), Create(diam), Create(chord), Create(arc), lag_ratio=0.18), run_time=RUN_SLOW * 1.5)
        self.play(FadeIn(labels), run_time=RUN_NORMAL)
        self.play(LaggedStart(*[FadeIn(card, shift=UP * 0.08) for card in route], lag_ratio=0.12), run_time=RUN_SLOW * 1.4)
        self.wait(COPY_PAUSE)
        self.clear_stage()

    def notebook_check(self) -> None:
        self.set_header(
            7,
            "NOTEBOOK CHECK",
            "Read each clue and say the element before the answer appears.",
        )
        questions = [
            ("Starts at O and ends on the circumference.", "RADIUS"),
            ("Joins two points and passes through O.", "DIAMETER"),
            ("Joins two points but may miss O.", "CHORD"),
            ("Curved part of the circumference.", "ARC"),
            ("Fixed point equally distant from the circumference.", "CENTER"),
        ]
        holder = RoundedRectangle(
            width=12.2,
            height=4.2,
            corner_radius=0.15,
            stroke_color=BLACK_LINE,
            stroke_width=2.2,
            fill_color=WHITE,
            fill_opacity=1.0,
        ).move_to(DOWN * 0.35)
        self.play(FadeIn(holder), run_time=RUN_NORMAL)
        current = None
        for i, (q, a) in enumerate(questions, start=1):
            idx = self.text(f"CHECK {i} / 5", 27, BOLD).next_to(holder.get_top(), DOWN, buff=0.28)
            prompt = self.text(q, 34, BOLD)
            self.fit(prompt, 10.7, 0.85)
            prompt.move_to(holder).shift(UP * 0.28)
            answer = self.text(a, 43, BOLD).next_to(prompt, DOWN, buff=0.48)
            state = VGroup(idx, prompt)
            if current is None:
                self.play(FadeIn(state), run_time=RUN_NORMAL)
            else:
                self.play(ReplacementTransform(current, state), run_time=RUN_NORMAL)
            self.wait(THINK_PAUSE)
            self.play(FadeIn(answer), run_time=RUN_NORMAL)
            self.wait(PAUSE_READ)
            self.play(FadeOut(answer), run_time=RUN_QUICK)
            current = state
        self.wait(PAUSE_FINAL)
        self.remove(*list(self.mobjects))
        self.header_group = None
        self.subtitle_group = None
        closing = self.text(
            "Center -> radius -> diameter -> chord -> arc",
            42,
            BOLD,
        )
        self.fit(closing, 13.5, 1.15)
        self.play(FadeIn(closing), run_time=RUN_SLOW)
        self.wait(PAUSE_FINAL)
        self.play(FadeOut(closing), run_time=RUN_NORMAL)


# Preview QA:
# LESSON_TIME_SCALE=0.08 manim -pql Geometry8_Circle_Fundamentals_Notebook.py Geometry8CircleFundamentalsNotebook --fps 15 --disable_caching
# Final:
# LESSON_TIME_SCALE=1.0 manim -pqh Geometry8_Circle_Fundamentals_Notebook.py Geometry8CircleFundamentalsNotebook --fps 30 --disable_caching
