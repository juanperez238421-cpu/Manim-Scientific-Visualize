...
class Physics9RelativeMotionHistoryV4(Scene):
    ...
    def train(self, width=9.5, height=2.35, exterior=False):
        body = RoundedRectangle(width=width, height=height, corner_radius=0.18,
                                stroke_color=INK, stroke_width=2.3,
                                fill_color=WHITE, fill_opacity=1.0)
        floor = Line(body.get_left() + RIGHT * 0.35 + DOWN * 0.73,
        ...
        for frac in (-0.34, -0.17, 0.0, 0.17, 0.34):
            win = RoundedRectangle(width=window_w, height=0.56, corner_radius=0.07,
                                   stroke_color=LIGHT, stroke_width=1.3,
                                   fill_color=PAPER, fill_opacity=1.0)
            win.move_to(body.get_center() + RIGHT * (frac * width) + UP * 0.56)
            windows.add(win)
        door = VGroup(Line(UP * 0.86, DOWN * 0.86, color=LIGHT, stroke_width=1.4),
        ...
        if exterior:
            wheels = VGroup(Circle(radius=0.16, stroke_color=INK, fill_color=WHITE, fill_opacity=1),
    ...
    def opening(self):
        title = VGroup(self.txt("PHYSICS 9 • RELATIVITY", 24, BOLD, DARK),
                       self.fit(self.txt("THE SAME MOTION CAN HAVE TWO VELOCITIES", 46, BOLD), 13.8),
                       self.txt("Reference frames first. Light comes later.", 27, NORMAL, DARK)).arrange(DOWN, buff=0.18).shift(UP * 2.45)
        ...
        arrow = self.velocity_arrow(LEFT * 4.0 + DOWN * 2.55, LEFT * 0.5 + DOWN * 2.55, r"20\,\mathrm{m/s}")
        self.play(GrowArrow(arrow[0]), Write(arrow[1]), run_time=RUN)
        self.wait(PAUSE_READ)
        self.clear_scene()

    def reference_frames(self):
        h = self.header(1, "WHO IS MEASURING THE MOTION?", "Velocity is always measured relative to a reference frame.")
    ...
    def inside_train(self):
        h = self.header(2, "INSIDE THE TRAIN: THE TRAIN IS YOUR REFERENCE FRAME", "You are seated. The walker moves at 2 m/s relative to you.")
        self.play(FadeIn(h), run_time=RUN_FAST)
        tr = self.train(width=8.2, height=2.45).move_to(LEFT * 3.05 + DOWN * 0.35)
        you = self.person(0.90, seated=True).move_to(tr.get_center() + LEFT * 2.35 + DOWN * 0.03)
        walker = self.person(0.90, walking=True).move_to(tr.get_center() + LEFT * 0.85 + DOWN * 0.03)
        you_lab = self.txt("YOU", 21, BOLD).next_to(you, DOWN, buff=0.08)
        walk_lab = self.txt("WALKER", 21, BOLD).next_to(walker, DOWN, buff=0.08)
        eq = self.formula_box(r"X'=X'_0+v't", width=5.0, size=41).move_to(RIGHT * 4.15 + UP * 1.25)
        sub = self.formula_box(r"X'=0+(2)(3)=6\,\mathrm{m}", width=5.0, size=36)
        sub.next_to(eq, DOWN, buff=0.22)
        result = self.result_box("INSIDE TRAIN: 2 m/s", width=5.0)
        result.next_to(sub, DOWN, buff=0.22)
        self.play(FadeIn(tr), FadeIn(you), FadeIn(walker), FadeIn(you_lab), FadeIn(walk_lab), run_time=RUN)
        self.play(FadeIn(eq), run_time=RUN)
        start = walker.get_center().copy()
        self.play(walker.animate.shift(RIGHT * 3.65), walk_lab.animate.shift(RIGHT * 3.65), run_time=3.0, rate_func=linear)
        dist = DoubleArrow(start + DOWN * 1.18, walker.get_center() + DOWN * 1.18, buff=0, color=MID, stroke_width=2.6)
    ...
    def from_building(self):
        h = self.header(3, "FROM THE BUILDING: THE TRAIN AND THE WALKER BOTH MOVE", "The train moves at 20 m/s. The walker also moves 2 m/s inside the train.")
        self.play(FadeIn(h), run_time=RUN_FAST)
        ground = Line(LEFT * 7.2 + DOWN * 2.0, RIGHT * 7.2 + DOWN * 2.0, color=MID, stroke_width=2)
        b = self.building().scale(0.76).to_edge(RIGHT, buff=0.55).shift(DOWN * 0.25)
        obs = self.person(0.62).move_to(b.get_center() + UP * 0.18)
        lab = self.txt("BUILDING OBSERVER", 18, BOLD).move_to(RIGHT * 5.65 + DOWN * 2.45)
        tr = self.train(width=6.7, height=1.86, exterior=True).move_to(LEFT * 4.75 + DOWN * 0.75)
        walker = self.person(0.68, walking=True).move_to(tr.get_center() + LEFT * 0.72 + DOWN * 0.04)
        self.play(Create(ground), FadeIn(b), FadeIn(obs), FadeIn(lab), FadeIn(tr), FadeIn(walker), run_time=RUN)
        vadd = self.formula_box(r"v_{\mathrm{walker,ground}}=20+2=22\,\mathrm{m/s}", width=7.0, size=35).move_to(LEFT * 1.55 + UP * 1.55)
        self.play(FadeIn(vadd), run_time=RUN)
        self.play(tr.animate.shift(RIGHT * 5.55), walker.animate.shift(RIGHT * 6.10), run_time=3.2, rate_func=linear)
        self.wait(PAUSE)
        self.play(FadeOut(vadd), FadeOut(tr), FadeOut(walker), run_time=RUN_FAST)
        positions = VGroup(self.formula_box(r"X_{\mathrm{train}}=0+(20)(3)=60\,\mathrm{m}", width=6.5, size=34),
                           self.formula_box(r"X_{\mathrm{walker}}=0+(22)(3)=66\,\mathrm{m}", width=6.5, size=34)).arrange(DOWN, buff=0.22).move_to(LEFT * 1.25 + UP * 0.55)
        self.play(FadeIn(positions[0]), run_time=RUN)
        self.play(FadeIn(positions[1]), run_time=RUN)
        diff = self.result_box("AFTER 3 s: 66 m − 60 m = 6 m", width=6.5)
        diff.next_to(positions, DOWN, buff=0.25)
        self.play(FadeIn(diff), run_time=RUN)
        self.wait(PAUSE_COPY)
        self.clear_scene()

    def compare_frames(self):
        h = self.header(4, "SAME WALKER. TWO CORRECT VELOCITIES.", "Nothing contradictory happened: the observers used different reference frames.")
        self.play(FadeIn(h), run_time=RUN_FAST)
        ...
        statement = self.fit(self.txt("CLASSICAL RELATIVITY: measured motion depends on the observer's frame.", 31, BOLD), 13.8).to_edge(DOWN, buff=0.45)
        self.play(Write(statement), run_time=RUN_SLOW)
        self.wait(PAUSE_READ)
        self.clear_scene()

    def classical_history(self):
        h = self.header(5, "GALILEO AND NEWTON: CLASSICAL MECHANICS", "For ordinary motion, Galilean velocity addition works extremely well.")
        self.play(FadeIn(h), run_time=RUN_FAST)
        years = self.txt("GALILEO  →  NEWTON 1687", 34, BOLD).shift(UP * 1.65)
        eq = self.formula_box(r"v=v'+V", width=4.8, size=50).shift(UP * 0.45)
        bullets = VGroup(self.txt("• The laws of mechanics work in inertial frames.", 28),
                         self.txt("• Newtonian time is treated as universal.", 28),
                         self.txt("• Velocities add and subtract between frames.", 28)).arrange(DOWN, aligned_edge=LEFT, buff=0.20).shift(DOWN * 1.20)
    ...
    def maxwell(self):
        h = self.header(6, "JAMES CLERK MAXWELL: LIGHT IS ELECTROMAGNETISM", "In the 1860s Maxwell unified electricity and magnetism.")
        self.play(FadeIn(h), run_time=RUN_FAST)
        name = self.txt("JAMES CLERK MAXWELL", 38, BOLD).shift(UP * 1.70)
        sentence = self.fit(self.txt("Maxwell's equations predict electromagnetic waves with a fixed vacuum wave speed.", 27), 13.2).shift(UP * 1.05)
        axis = Line(LEFT * 5.5, RIGHT * 5.5, color=MID, stroke_width=2).shift(DOWN * 0.55)
        wave = ParametricFunction(lambda t: np.array([t, 0.55 * np.sin(2.3 * t), 0]), t_range=[-5.0, 5.0], color=AMBER, stroke_width=4).shift(DOWN * 0.55)
        cbox = self.formula_box(r"c\approx 3.00\times10^8\,\mathrm{m/s}", width=6.0, size=42).shift(DOWN * 2.00)
    ...
    def tension(self):
        h = self.header(7, "THE PROBLEM: CLASSICAL VELOCITY ADDITION MEETS MAXWELL", "Galilean kinematics and Maxwell electromagnetism do not transform the same way.")
        self.play(FadeIn(h), run_time=RUN_FAST)
        left = RoundedRectangle(width=6.2, height=4.3, corner_radius=0.16, stroke_color=DARK, stroke_width=2, fill_color=WHITE, fill_opacity=1).shift(LEFT * 3.45 + DOWN * 0.20)
        ...
        ltxt = self.fit(self.txt("Applied to light, this would suggest c + V or c − V.", 24), 5.45).move_to(left.get_center() + DOWN * 0.85)
        req = self.mtex(r"c=\mathrm{constant\ wave\ speed}", 38).move_to(right.get_center() + UP * 0.35)
        rtxt = self.fit(self.txt("Maxwell's equations are not Galilean-invariant.", 24), 5.45).move_to(right.get_center() + DOWN * 0.85)
        ...
        conflict = self.fit(self.txt("A NEW KINEMATICS WAS NEEDED.", 34, BOLD), 11.5).to_edge(DOWN, buff=0.38)
        self.play(Write(conflict), run_time=RUN_SLOW)
        self.wait(PAUSE_READ)
        self.clear_scene()

    def einstein(self):
        h = self.header(8, "EINSTEIN 1905: RECONCILING RELATIVITY WITH ELECTROMAGNETISM", "Einstein did not discard Maxwell; he changed our ideas of space and time.")
        self.play(FadeIn(h), run_time=RUN_FAST)
        ...
        consequence = self.fit(self.txt("If c does not change between frames, space and time cannot both remain absolute.", 30, BOLD), 13.4).shift(DOWN * 1.35)
    ...
    def final_bridge(self):
        h = self.header(9, "ONE STORY: FROM A WALKER TO SPECIAL RELATIVITY", "Start with reference frames. Then ask what happens when the moving signal is light.")
        self.play(FadeIn(h), run_time=RUN_FAST)
        ...
        matter = VGroup(self.txt("WALKER", 25, BOLD), self.txt("Train frame: 2 m/s", 25), self.txt("Building frame: 22 m/s", 25)).arrange(DOWN, aligned_edge=LEFT, buff=0.12).move_to(LEFT * 4.65 + DOWN * 2.05)
        light = VGroup(self.txt("LIGHT", 25, BOLD), self.mtex(r"c\ \mathrm{inside}", 31), self.mtex(r"c\ \mathrm{outside}", 31)).arrange(DOWN, aligned_edge=LEFT, buff=0.12).move_to(RIGHT * 3.55 + DOWN * 2.05)
        ...
        final = self.fit(self.txt("RELATIVITY: measurements depend on the frame — the laws of physics do not.", 31, BOLD), 13.6).to_edge(DOWN, buff=0.20)
