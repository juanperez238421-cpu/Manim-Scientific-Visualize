...
SCENE=render_jobs/geometry8_circle_exercises_20260824/Geometry8_Circle_Exercises_Workshop_20260824_V5_LargeText_QA.py
CLASS=Geometry8CircleExercisesWorkshop20260824V5LargeTextQA
mkdir -p "$JOB" /tmp/manim-bin /tmp/manim-home diag_media_v5 diag_v5
...
set +e
manim -pql "$SCENE" "$CLASS" --fps 15 --format=mp4 --media_dir diag_media_v5 --disable_caching --progress_bar none > diag_v5/full.log 2>&1
status=$?
set -e
if [ "$status" -ne 0 ]; then
  echo '=== CONCISE MANIM FAILURE ==='
  grep -E -B 10 -A 45 'Traceback|ValueError|TypeError|AttributeError|IndexError|KeyError|Exception|ERROR|exceeds|Error|padding|safe' diag_v5/full.log | tail -n 180 || tail -n 180 diag_v5/full.log
