...
'''    def uml_generalization(self, child: Mobject, parent: Mobject, target_shift=ORIGIN) -> VGroup:\n        """Hollow-triangle UML generalization with an explicit parent-side anchor."""\n        start = child.get_top()\n        end = parent.get_bottom() + target_shift\n'''))
...
'''    def uml_realization(self, child: Mobject, interface: Mobject, target_shift=ORIGIN) -> VGroup:\n        """Dashed UML realization with a distinct interface-side anchor."""\n        start = child.get_top()\n        end = interface.get_bottom() + target_shift\n'''))
...
'''        relations = VGroup(\n            self.uml_generalization(robot, parent, LEFT*0.58),\n            self.uml_generalization(drone, parent, RIGHT*0.58),\n        )\n'''))

repls.append((
'''        relations = VGroup(*[self.uml_realization(c, interface) for c in classes])\n''',
'''        relations = VGroup(\n            self.uml_realization(robot, interface, LEFT*0.72),\n            self.uml_realization(drone, interface, ORIGIN),\n            self.uml_realization(boat, interface, RIGHT*0.72),\n        )\n'''))
...
new_project = '''        scenario = self.text("Smart garden: read soil moisture and activate a pump when water is needed.", 24, BOLD).move_to(UP*2.35)\n        controller = self.uml_class("Controller", ["- threshold : float"], ["+ evaluate()"], width=3.75, height=2.55, body_size=16).move_to(UP*0.80)\n        sensor = self.uml_class("MoistureSensor", ["- value : float"], ["+ read()"], width=3.75, height=2.55, body_size=16).move_to(LEFT*4.45 + DOWN*2.05)\n        pump = self.uml_class("Pump", ["- isOn : bool"], ["+ start()", "+ stop()"], width=3.75, height=2.55, body_size=16).move_to(DOWN*2.05)\n        garden = self.uml_class("Garden", ["- name : str"], ["+ irrigate()"], width=3.75, height=2.55, body_size=16).move_to(RIGHT*4.45 + DOWN*2.05)\n        classes = VGroup(controller, sensor, pump, garden)\n\n        # Classical UML-style associations: simple solid lines, labels offset from connectors.\n        sensor_link = Line(sensor.get_top()+UP*0.02, controller.get_left()+DOWN*0.28, color=BLACK_LINE, stroke_width=1.8)\n        sensor_label = self.text("reads", 16, MEDIUM).move_to(sensor_link.get_center() + UP*0.28)\n        pump_link = Line(controller.get_bottom()+DOWN*0.02, pump.get_top()+UP*0.02, color=BLACK_LINE, stroke_width=1.8)\n        pump_label = self.text("controls", 16, MEDIUM).move_to(pump_link.get_center() + RIGHT*0.70)\n        garden_link = Line(pump.get_right()+RIGHT*0.02, garden.get_left()+LEFT*0.02, color=BLACK_LINE, stroke_width=1.8)\n        garden_label = self.text("irrigates", 16, MEDIUM).move_to(garden_link.get_center() + UP*0.26)\n        links = VGroup(VGroup(sensor_link, sensor_label), VGroup(pump_link, pump_label), VGroup(garden_link, garden_label))\n'''
...
print('V11 visual QA patch applied')
