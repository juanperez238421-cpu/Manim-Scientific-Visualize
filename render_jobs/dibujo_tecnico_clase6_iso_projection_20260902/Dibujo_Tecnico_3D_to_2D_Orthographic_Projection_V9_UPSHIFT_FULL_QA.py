#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Dibujo Técnico y CAD — V9 UPSHIFT FULL QA.
...
V9 preserves the validated stepped solid, FRONT/TOP/RIGHT projections, PV/PH
...
class Projection3Dto2DV9UpShiftFullQA(Projection3Dto2DFullTotalV8):
    ...
    def construct(self):
        ...
        self.fixed_fade_in(cue_f, run_time=T(0.40))
        front_sources = [
        ...
        self.play(LaggedStart(*[Create(r) for r in front_rays], lag_ratio=0.055), run_time=T(1.55))
        self.wait(T(0.45))
        self.play(Create(front), run_time=T(1.05))
        self.play(Indicate(front, color=FRONT_COLOR, scale_factor=1.015), run_time=T(0.85))
        self.wait(T(1.55))
        self.play(FadeOut(front_rays), run_time=T(0.60))
        self.fixed_fade_out(cue_f, run_time=T(0.22))

        # ------------------------------------------------------------------
        # 5 · TOP → PLANTA
        # ------------------------------------------------------------------
        cue_t = self.section_chip("4 · TOP → PLANTA", 3.60)
        self.fixed_fade_in(cue_t, run_time=T(0.40))
        top_sources = [
        ...
        self.play(LaggedStart(*[Create(r) for r in top_rays], lag_ratio=0.045), run_time=T(1.65))
        self.wait(T(0.45))
        self.play(Create(top), run_time=T(1.05))
        self.play(Indicate(top, color=TOP_COLOR, scale_factor=1.012), run_time=T(0.85))
        self.wait(T(1.55))
        self.play(FadeOut(top_rays), run_time=T(0.60))
        self.fixed_fade_out(cue_t, run_time=T(0.22))

        # ------------------------------------------------------------------
        # 6 · RIGHT → PERFIL
        # ------------------------------------------------------------------
        cue_r = self.section_chip("5 · RIGHT → PERFIL", 3.80)
        self.fixed_fade_in(cue_r, run_time=T(0.40))
        right_sources = [
        ...
        self.play(LaggedStart(*[Create(r) for r in right_rays], lag_ratio=0.06), run_time=T(1.55))
        self.wait(T(0.45))
        self.play(Create(right), run_time=T(1.05))
        self.play(Indicate(right, color=RIGHT_COLOR, scale_factor=1.012), run_time=T(0.85))
        self.wait(T(1.55))
        self.play(FadeOut(right_rays), run_time=T(0.60))
        self.fixed_fade_out(cue_r, run_time=T(0.22))

        # Hold the complete physical construction in the corrected upper frame.
        complete = self.section_chip("MISMO OBJETO 3D → TRES PROYECCIONES 2D", 6.10)
        self.fixed_fade_in(complete, run_time=T(0.40))
        complete_center = self.lifted_center(0.0, 1.70, 1.45)
        self.move_camera(
            theta=-61 * DEGREES, phi=61 * DEGREES, zoom=0.95,
            frame_center=complete_center, run_time=T(1.45),
        )
        self.wait(T(0.55))
        self.move_camera(
            theta=-48 * DEGREES, phi=65 * DEGREES, zoom=0.98,
            frame_center=complete_center, run_time=T(1.35),
        )
        self.wait(T(1.70))
        self.fixed_fade_out(complete, run_time=T(0.22))

        # ------------------------------------------------------------------
        # 7 · REMOVE SOURCE BODY
        # ------------------------------------------------------------------
        keep = self.section_chip("6 · RETIRAMOS EL OBJETO · LAS VISTAS PERMANECEN", 6.45)
        self.fixed_fade_in(keep, run_time=T(0.40))
        self.play(FadeOut(solid), run_time=T(1.00))
        self.wait(T(1.25))
        self.fixed_fade_out(keep, run_time=T(0.22))
        self.play(FadeOut(side), FadeOut(right), pr_lab.animate.set_opacity(0), run_time=T(0.70))
        self.wait(T(0.45))

        # ------------------------------------------------------------------
        # 8 · LITERAL PH ABATIMIENTO
        # ------------------------------------------------------------------
        unfold = self.section_chip("7 · ABATIMIENTO · PH GIRA 90° SOBRE LT", 5.55)
        self.fixed_fade_in(unfold, run_time=T(0.40))
        self.play(
            Rotate(
                VGroup(ph, top),
                angle=-PI / 2,
        ...
        self.wait(T(1.35))
        self.fixed_fade_out(unfold, run_time=T(0.22))

        # ------------------------------------------------------------------
        # 9 · ALIGNMENT GUIDES
        # ------------------------------------------------------------------
        align = self.section_chip("8 · LAS MEDIDAS COMUNES QUEDAN ALINEADAS", 5.65)
        self.fixed_fade_in(align, run_time=T(0.40))
        guides = VGroup(*[
            Line(
        ...
        self.play(LaggedStart(*[Create(g) for g in guides], lag_ratio=0.08), run_time=T(1.25))
        self.wait(T(1.80))
        self.play(FadeOut(guides), run_time=T(0.50))
        self.fixed_fade_out(align, run_time=T(0.22))
        ...
        # ------------------------------------------------------------------
        # 10 · CLEAN 2D DRAWING SHEET — panels lifted, heading remains safe.
        # ------------------------------------------------------------------
        sheet_title = Text("UN OBJETO · TRES DESCRIPCIONES 2D", font_size=35, color=INK, weight=BOLD)
        ...
        self.play(sheet_title.animate.set_opacity(1), run_time=T(0.55))
        self.wait(T(0.45))
        self.play(t_panel.animate.set_opacity(1), run_time=T(0.80))
        self.wait(T(0.45))
        self.play(vguide.animate.set_opacity(1), f_panel.animate.set_opacity(1), run_time=T(0.90))
        self.wait(T(0.45))
        self.play(hguide.animate.set_opacity(1), r_panel.animate.set_opacity(1), run_time=T(0.90))
        self.wait(T(2.45))
        self.play(sheet_group.animate.set_opacity(0), run_time=T(0.70))
        self.remove_fixed_in_frame_mobjects(sheet_group)

        # ------------------------------------------------------------------
        # 11 · FIVE-STEP METHOD — whole teaching block shifted upward.
        # ------------------------------------------------------------------
        ...
        method.move_to(ORIGIN + UP * SCREEN_GROUP_UPSHIFT)
        self.add_fixed_in_frame_mobjects(method)
        method.set_opacity(0)
        self.play(method_title.animate.set_opacity(1), run_time=T(0.55))
        for bar in bars:
            bar.set_opacity(0)
        for bar in bars:
            self.play(bar.animate.set_opacity(1), run_time=T(0.45))
            self.play(Circumscribe(bar[0], color=FRONT_COLOR, buff=0.04, time_width=0.35), run_time=T(0.65))
            self.wait(T(0.50))
        self.wait(T(1.70))
        self.play(method.animate.set_opacity(0), run_time=T(0.65))
        self.remove_fixed_in_frame_mobjects(method)

        # ------------------------------------------------------------------
        # 12 · CLOSING SYNTHESIS — raised, but intentionally sparse.
        # ------------------------------------------------------------------
        ...
        final.set_opacity(0)
        self.play(final.animate.set_opacity(1), run_time=T(0.90))
        self.wait(T(3.40))
        self.play(final.animate.set_opacity(0), run_time=T(0.75))
        self.remove_fixed_in_frame_mobjects(final)


# Preview:
# manim -pql Dibujo_Tecnico_3D_to_2D_Orthographic_Projection_V9_UPSHIFT_FULL_QA.py Projection3Dto2DV9UpShiftFullQA --disable_caching
# Final:
# manim -pqh Dibujo_Tecnico_3D_to_2D_Orthographic_Projection_V9_UPSHIFT_FULL_QA.py Projection3Dto2DV9UpShiftFullQA --disable_caching
