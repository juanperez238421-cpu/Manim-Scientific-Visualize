...
SCENE=OpenAIAssetSafeDemo
OUT=render_outputs/openai_asset_demo_20260815
mkdir -p assets/images delivery qa "$OUT"
cp "$JOB/demo_safe.py" main.py
...
P="$(find media -type f -name "$SCENE.mp4" -print -quit)"
test -n "$P" && test -s "$P"
ffmpeg -nostdin -v error -i "$P" -f null -

render -pqh 1.0 final.log
V="$(find media -type f -name "$SCENE.mp4" -print -quit)"
test -n "$V" && test -s "$V"
...
printf 'Scene: %s\nManimCE: 0.20.1\nPreview: manim -pql main.py %s --format=mp4 --disable_caching\nFinal: manim -pqh main.py %s --format=mp4 --disable_caching\nResolution: %sx%s\nFPS: %s\nCodec: %s\nPixel format: %s\nDuration: %s\nSize bytes: %s\n' "$SCENE" "$SCENE" "$SCENE" "$W" "$H" "$FPS" "$CODEC" "$PIX" "$DUR" "$SIZE" > delivery/RENDER_INFO.txt

cp delivery/${SCENE}_pqh.mp4 "$OUT/"
cp delivery/VIDEO_TECHNICAL.tsv delivery/VIDEO_SHA256.txt delivery/RENDER_INFO.txt "$OUT/"
