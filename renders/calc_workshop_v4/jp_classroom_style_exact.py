...
class JPClassroomScene(MovingCameraScene):
    ...
    def focus_on(self, mob: Mobject, width: float = 8.0, pause: float = PAUSE_READ) -> None:
        persistent = [item for item in (self.header_group, self.subtitle_group) if item is not None]
        if persistent:
            self.play(*[FadeOut(item) for item in persistent], run_time=RUN_QUICK)
        self.camera.frame.save_state()
        target_width = max(width, mob.width + 0.8)
        self.play(self.camera.frame.animate.set(width=target_width).move_to(mob), run_time=RUN_CAMERA)
        self.wait(pause)
    ...
    def build_table(self, headers: Sequence[str], body_rows: Sequence[Sequence[str]], column_widths: Sequence[float], *, math_columns: Iterable[int] = (), row_height: float = 0.62, header_height: float = 0.72, body_font_size: int = 25, header_font_size: int = 23) -> TableDiagram:
        ...
        for row_index in range(row_count):
            ...
            for column_index, column_width in enumerate(column_widths):
                ...
                rectangle = Rectangle(width=column_width, height=current_height, stroke_color=BLACK_LINE, stroke_width=1.25, fill_color=VERY_LIGHT_GRAY if is_header else WHITE_FILL, fill_opacity=1.0)
                ...
                if is_header:
                    ...
                    entry = self.math(content, header_font_size) if is_math_header else self.text(content, header_font_size, BOLD)
        ...
        for column_index in range(column_count):
            column_items = []
            for row_index in range(row_count):
                column_items.extend([rectangles[row_index][column_index], entries[row_index][column_index]])
        ...
        return TableDiagram(group=full_group, rectangles=rectangles, entries=entries, rows=row_groups, columns=column_groups, header=header_group, body=body_group)

    def shade_cells(self, table: TableDiagram, coordinates: Sequence[tuple[int, int]], opacity: float = 1.0) -> AnimationGroup:
        animations = [table.rectangles[r][c].animate.set_fill(LIGHT_GRAY, opacity=opacity) for r, c in coordinates]
    ...
    def figure_panel(self, figure: Mobject, *, width: float = 6.2, height: float = 4.5, title: str | None = None, caption: str | None = None, inner_margin: float = 0.38, title_size: int = 25, caption_size: int = 19, fill_color=WHITE_FILL) -> FigurePanel:
        box = RoundedRectangle(width=width, height=height, corner_radius=0.12, stroke_color=BLACK_LINE, stroke_width=1.8, fill_color=fill_color, fill_opacity=1.0)
        ...
        return FigurePanel(VGroup(*components), box, figure, title_mob, caption_mob)

    def image_panel(self, image_path: str | Path, **kwargs) -> FigurePanel:
        path = Path(image_path)
        if path.is_absolute():
            raise ValueError(f"Use project-relative asset paths, not absolute paths: {path}")
        if not path.exists():
            raise FileNotFoundError(path)
        return self.figure_panel(ImageMobject(str(path)), **kwargs)
    ...
    def transform_equation_chain(self, expressions: Sequence[str], *, size: int = 42, position: np.ndarray = ORIGIN, pause: float = PAUSE_EXPLAIN) -> Mobject:
        if not expressions:
    ...
    def process_map(self, steps: Sequence[tuple[str, str]], *, card_width: float = 4.3, card_height: float = 1.10, columns: int = 3) -> VGroup:
        cards = VGroup()
        for number, text_value in steps:
            badge = RoundedRectangle(width=0.66, height=0.50, corner_radius=0.08, stroke_color=BLACK_LINE, stroke_width=1.5, fill_color=VERY_LIGHT_GRAY, fill_opacity=1)
            ...
            box = RoundedRectangle(width=card_width, height=card_height, corner_radius=0.10, stroke_color=BLACK_LINE, stroke_width=1.5, fill_color=WHITE_FILL, fill_opacity=1)
            content.move_to(box)
